...
def encoder_2(x):
    #input times weight + bias. Activate! It rhymes.
    # Encoder Hidden layer with sigmoid activation #1
    layer_2 = tf.nn.tanh(tf.add(tf.matmul(x, weights['encoder_h2']),
                                   biases['encoder_b2']))
    return layer_2
...
encoder_op_1 = encoder_1(X)
encoder_op_2 = encoder_2(encoder_op_1)

#and learned representation to the decoder
decoder_op_1 = decoder_1(encoder_op_2)
decoder_op_2 = decoder_2(encoder_op_1)
...
with tf.Session() as sess:
    ...
    test_encoding = sess.run(encoder_op_2, feed_dict={X: X_test})
    train_encoding = sess.run(encoder_op_2, feed_dict={X: X_train})
