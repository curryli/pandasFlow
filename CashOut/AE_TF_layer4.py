...
def decoder(x):
    # Encoder Hidden layer with sigmoid activation #1
    layer_4 = tf.nn.tanh(tf.add(tf.matmul(x, weights['decoder_h4']),
                                   biases['decoder_b4']))
    
    layer_3 = tf.nn.tanh(tf.add(tf.matmul(layer_4, weights['decoder_h3']),
                                   biases['decoder_b3']))
    
    layer_2 = tf.nn.tanh(tf.add(tf.matmul(layer_3, weights['decoder_h2']),
                                   biases['decoder_b2']))
