...
Alldata = pd.read_csv('LSTM_converted_10.csv')
...
#    classifier.add(Dropout(0.8))
    ...
    classifier.add(LSTM(64, input_shape=(BS,timesteps, data_dim),   stateful=True, recurrent_dropout=0.3, activation='sigmoid',  recurrent_activation='hard_sigmoid',   unit_forget_bias=True, return_sequences=True))
    classifier.add(Dropout(0.7))
    classifier.add(LSTM(64, input_shape=(BS,timesteps, data_dim), stateful=True, recurrent_dropout=0.3, activation='sigmoid',  recurrent_activation='hard_sigmoid',   unit_forget_bias=True))
    classifier.add(Dropout(0.7))
...
class_weights= dict() 
class_weights[0] =  1.0     #这个是正常的
class_weights[1] = 5.0   #这个是欺诈的    
