...
Autoencoder.add(Dense(256, input_shape=(inputSize,)))  
Autoencoder.add(Activation('relu')) 
Autoencoder.add(Dropout(0.3))


Autoencoder.add(Dense(128, input_shape=(256,)))  
Autoencoder.add(Activation('relu')) 
Autoencoder.add(Dropout(0.3))

Autoencoder.add(Dense(64, input_shape=(128,)))  
Autoencoder.add(Activation('relu')) 
Autoencoder.add(Dropout(0.3))

Autoencoder.add(Dense(32, input_shape=(64,)))  
Autoencoder.add(Activation('relu')) 
Autoencoder.add(Dropout(0.3))
...
#进行训练：
Autoencoder.fit(X_train, y_train, nb_epoch=Epochs, shuffle=True, validation_data=(X_test, y_test))
 
   
pred = Autoencoder.predict(X_test) 
pred =  [j[0] for j in pred]
