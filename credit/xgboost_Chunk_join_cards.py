...
df_All = df_All.drop(["timestamp_in_min","timestamp_in_hour"], axis=1,inplace=False)

df_All = df_All[(df_All["label"] == 0) | (df_All["label"] == 1)]

print df_All.shape[0]
#df_All = df_All.sample(n=2000)
...
df_cards = df_All["certid"].drop_duplicates()
cards_train, cards_test = train_test_split(df_cards, test_size=0.2)


df_X = df_All.drop(["label","card_no"], axis=1,inplace=False)
df_y = df_All[["certid","label"]]


X_train = df_X[df_X.certid.isin(cards_train.values)]
X_test = df_X[df_X.certid.isin(cards_test.values)]
y_train = df_y[df_y.certid.isin(cards_train.values)]
y_test = df_y[df_y.certid.isin(cards_test.values)]
print df_y.shape, y_train.shape, y_test.shape
...
pred_label_DF = pred_label_DF.drop_duplicates()
