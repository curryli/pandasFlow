...
X_stat = df_All_train.drop(["certid", "label"], axis=1, inplace=False)
y_stat = df_All_train["label"]
X_cols = list(X_stat.columns.values)
print len(X_cols)
...
for i in range(0,20):
    ...
    IF_clf.fit(X_stat)
    y_pred_train = IF_clf.predict(X_stat)

    A = X_stat.values
    B = y_stat.reshape(len(y_stat),1)
