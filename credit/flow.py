...
if __name__ == '__main__':
    ...
    Trans_ori_df = pd.read_csv("train_trans_encrypt.csv",sep=",", low_memory=False, error_bad_lines=False)
    #Trans_ori_df = Trans_ori_df.fillna('NAN')

    Trans_ori_df = Trans_ori_df.fillna(0)

    Trans_ori_df = Trans_ori_df.replace('', 0)
...
##############################
    ...
    # Trans_df = pd.get_dummies(Trans_ori_df[['iss_ins_cd', 'trans_chnl',
    #                       'mchnt_cd', 'mcc_cd', 'card_accprt_nm_loc', 'resp_cd', 'trans_id_cd', 'orig_trans_st',
    #                       'trans_st', 'trans_curr_cd', 'fwd_settle_cruu_cd', 'fwd_settle_conv_rt', 'rcv_settle_curr_cd',
    #                       'rcv_settle_conv_rt', 'cdhd_curr_cd', 'cdhd_conv_rt', 'term_cd', 'card_attr_cd',
    #                       'card_media_cd', 'pos_cond_cd', 'pos_entry_md_cd', 'auth_id_resp_cd']])

    #Trans_df = pd.concat([Trans_ori_df[['certid','Trans_at', 'Trans_tm', 'fwd_settle_at', 'rcv_settle_at', 'cdhd_at', "Settle_dt"]], Trans_df], axis=1)
...
###############################
    ...
    for item in math_list:
        agg_dict[item] = ['min', 'max', 'mean', 'sum', 'median', 'std', 'var']
