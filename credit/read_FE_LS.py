...
#https://stackoverflow.com/questions/36880490/why-use-pandas-qcut-return-valueerror-bin-edges-must-be-unique
...
if __name__ == '__main__':
    #Trans_ori_df = pd.read_csv("small_data.csv", sep=",", low_memory=False, error_bad_lines=False)
    ...
    # Trans_ori_df['Trans_at'] = pd.qcut(Trans_ori_df['Trans_at'], 10)
    # Trans_ori_df['Trans_at'] = pd.factorize(Trans_ori_df.Trans_at)[0] + 1

########################离散化###################################
    for i in math_list:
        Trans_ori_df[i] = pct_rank_qcut(Trans_ori_df[i], 10)
        #Trans_ori_df[i] = pd.qcut(Trans_ori_df[i], 10)
        #Trans_ori_df[i] = pd.factorize(Trans_ori_df.Trans_at)[0] + 1
...
################################################Join 后再分析#######################################
 ...
 ####################地区详细分析###############
    ...
    risk_items = np.setdiff1d(Bad_items[:int(Bad_items.shape[0] * 0.1)], Good_items[:int(Bad_items.shape[0] * 0.1)])
...
###############################################
    Effect_df.to_csv("train_1110_LS.csv",index=False)
