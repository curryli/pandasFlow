...
if __name__ == '__main__':
    Trans_ori_df = pd.read_csv("Trans_ori_idx.csv", sep=",", low_memory=False, error_bad_lines=False)
...
####################################################################################################
    ...
    agg_dict['Trans_at'] = ['count', 'min', 'max', 'mean', 'sum', 'median', 'std', 'var', peak_to_peak]   #覆盖
 ...
 ############################################
    ...
    Effect_df.to_csv("train_spark_idx.csv",index=False)
###############################################
