...
train_all = pd.read_csv('train-converted_2.csv')
test_all = pd.read_csv('train-converted_2.csv')
...
for i in range(0, runLoop):
    hist=classifier.fit(X_train, y_train, batch_size=256, epochs=runEpoch, class_weight=class_weights, validation_data=(X_test, y_test))
    print "loop i=", i, "hist:", hist.history
    y_predict=classifier.predict(X_test,batch_size=256)
