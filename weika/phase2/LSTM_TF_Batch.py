...
with tf.Session() as sess:
    ...
    for e in range(runEpoch):
        ...
        while(i<train_size):
            ...
            if (i+1)%BS == 0:
                step = (i+1)/BS
                train_batch_mse = sess.run(batch_mse, feed_dict={_X:_X_reshape, _y: _y_reshape, dropout: 0.4, batch_size: BS})
                print "Epoch:", e, "step:",step, " cost=", "{:.9f}".format(cost),  "Train auc=",  auc(batch[1], y_prediction)
