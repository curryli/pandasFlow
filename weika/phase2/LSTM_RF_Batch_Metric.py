...
def f1_pred(y_test,y_predict):
    CM_test = confusion_matrix(y_test,y_predict)
    precision_p = float(CM_test[1][1])/float((CM_test[0][1] + CM_test[1][1]))
    recall_p = float(CM_test[1][1])/float((CM_test[1][0] + CM_test[1][1]))
...
def classifier_builder ():
    ...
    classifier.compile(loss='binary_crossentropy',
              ...
              metrics=[metrics.mae,"accuracy",f1_pred])
