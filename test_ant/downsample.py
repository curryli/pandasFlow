...
#df_All["is_visit_freq_use"] = df_All["visit_freq"].map(lambda x: 1 if(x in [4,5,6,9]) else 0)
...
y_test = df_V['y_buy']
clf = RandomForestClassifier(n_estimators=200, max_depth=3, min_samples_split=2, max_features="auto", max_leaf_nodes=None, bootstrap=True)
...
#例如在反欺诈场景，设非欺诈类样本为正例，负例占比很少（假设0.1%），如果使用准确率评估，把所有的样本预测为正例便可以获得99.9%的准确率。但是如果使用AUC，把所有样本预测为正例，TPRate和FPRate同时为1，AUC仅为0.5，成功规避了样本不均匀带来的问题。
#例如在反欺诈场景，设非欺诈类样本为正例，负例占比很少（假设0.1%），如果使用准确率评估，把所有的样本预测为正例便可以获得99.9%的准确率。但是如果使用AUC，把所有样本预测为正例，TPRate和FPRate同时为1，AUC仅为0.5，说明效果一般。
